...
public class BoardController2 {
	...
	public ResponseEntity<List<MemberVO>> listArticles() {
		...
		// MemberVO 타입으로만, 박스에 담아서 전달 해주세요. 강력한 요구사항.
		for (int i = 0; i < 10; i++) {
	...
	public ResponseEntity<MemberVO> findArticle (@PathVariable("articleNO") Integer articleNO) {
		...
		logger.info("@PathVariable(\"articleNO\") : 서버에 잘 호출 되었는지 확인 "+articleNO);
		MemberVO vo = new MemberVO();
		vo.setId("lsy" );
		vo.setName("이상용");
		vo.setPwd("점심메뉴");
	...
	//@RequestBody MemberVO MemberVO : 서버에 전달된 데이터를, 모델 클래스에 자동 매핑.
